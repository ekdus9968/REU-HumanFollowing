...
lateral_pid = PID(Kp=40.0,  Ki=0.0, Kd=2.0,   output_limit=MAX_SPEED)
forward_pid = PID(Kp=30.0, Ki=0.0, Kd=0.005, output_limit=MAX_SPEED)
...
def determine_state(gesture, color_det, hand_det, target_found):
    ...
    if gesture == "CLOSE" or gesture == "UD_Close":
        return State.STOP
    if not target_found:
        return State.IDLE
